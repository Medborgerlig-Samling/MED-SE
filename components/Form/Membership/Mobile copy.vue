<template>
  <v-card flat color="white" variant="tonal" rounded="0" class="pa-2 justify-center" width="100vw" max-width="100%">
    <v-form v-model="valid" validate-on="invalid-input" @submit.prevent="onSubmit">
      <v-container>
        <v-row>
        ...
        <v-sheet class="mb-4 rounded-lg d-flex flex-column" color="transparent" variant="tonal">
          ...
          <v-checkbox
            ...
            @update:model-value="$emit('update:isRecurring', $event)"
          />
        </v-sheet>

        <v-btn type="submit" color="accent" block :disabled="!valid || isLoading">
